...
namespace LiveBetApp.Services
{
    public class CoreServiceV2
    {
        ...
        private List<string> _backuptimes;

        public CoreServiceV2()
        {
            _rebuildUrlService = new RebuildUrlService();
            _reconnectCount = 0;
            _backuptimes = new List<string>();
        }
        ...
        public void ExecuteCore()
        {
            ...
            using (var ws = new WebSocket(webSocketUrl))
            {
                ...
                ws.OnMessage += (sender, e) =>
                {
                    if (e.Data == "40")
                    {
                        ws.Send("42[\"init\",{\"gid\":\"" + _rebuildUrlService.Gid + "\",\"token\":\"" + _rebuildUrlService.Token + "\",\"id\":\"" + _rebuildUrlService.Id + "\",\"rid\":\"" + _rebuildUrlService.Rid + "\",\"v\":2}]");
                        ws.Send("42[\"subscribe\",[[\"odds\",[{\"id\":\"c2\",\"rev\":\"\",\"sorting\":\"n\",\"condition\":{\"sporttype\":1,\"marketid\":\"L\",\"bettype\":[1,3,5,7,8,15,301,302,303,304]}},{\"id\":\"c3\",\"rev\":\"\",\"sorting\":\"n\",\"condition\":{\"sporttype\":1,\"marketid\":\"T\",\"bettype\":[1,3,5,7,8,15,301,302,303,304]}}]]]]");
                        //
                        _reconnectCount = 0;
                    }
                    else if (e.Data.Length > 6)
                    {
                        try
                        {
                            ProcessWebSocketData(JArray.Parse(e.Data.Substring(2, e.Data.Length - 2)));
                        }
                        catch (Exception ex)
                        {
                            Common.Functions.WriteExceptionLog(new Exception("1"));
                        }
                    }
                };
                ...
                try
                {
                    ...
                    while (true)
                    {
                        ...
                        string nowStr = now.ToString("yyyyMMddHHmm");

                        if (now.Minute % 5 == 0 
                            && !DataStore.Backuptimes.Contains(nowStr))
                        {
                            DataStore.Backuptimes.Add(nowStr);
                            Common.Functions.BackUpDatastore(now);
                        }

                        if (now.Minute == DataStore.MinuteToBackUp)
                        {
                            if (!DataStore.Backuptimes.Contains(nowStr))
                            {
        ...
        private void ProcessWebSocketData(JToken data)
        {
            try
            {
                string groupName = data[1].ToString();
                ProcessGroupData(groupName, data[2]);
                DataStore.LastTick = DateTime.Now;
            }
            catch (Exception ex)
            {
                Common.Functions.WriteExceptionLog(new Exception("2"));
            }

        }

        private void ProcessGroupData(string groupName, JToken data)
        {
            int length = data.Count();
            for (int i = 0; i < length; i++)
            {
                ProcessGroupDataItem(groupName, data[i]);
            }
        }

        private void ProcessGroupDataItem(string groupName, JToken data)
        {
            if (data[0].Type == JTokenType.String)
            {
                ProcessPropertyMapping(groupName, data);
            }
            else // JTokenType.Integer
            {
                ProcessData(groupName, data);
            }
        }

        private void ProcessPropertyMapping(string groupName, JToken data)
        {
            if (data[0].ToString() == "f")
            {
                UpdatePropertyMapping(groupName, int.Parse(data[1].ToString()), data[2]);
            }
        }

        private void UpdatePropertyMapping(string propertyMappingName, int beginIndex, JToken data)
        {
            if (!DataStore.PropertyMapping.ContainsKey(propertyMappingName))
            {
                DataStore.PropertyMapping.Add(propertyMappingName, new Dictionary<int, string>());
            }
            int length = data.Count();
            for (int i = 0; i < length; i++)
            {
                int key = beginIndex + i;
                if (!DataStore.PropertyMapping[propertyMappingName].ContainsKey(key))
                {
                    DataStore.PropertyMapping[propertyMappingName].Add(key, data[i].ToString());
                }
            }
        }

        private void ProcessData(string groupName, JToken data)
        {
            int length = data.Count();
            JToken jTokenMapped = new JObject();
            for (int i = 0; i < length; i += 2)
            {
                string propertyName = DataStore.PropertyMapping[groupName][((int)data[i])];
                jTokenMapped[propertyName] = data[i + 1];
            }
            ProcessCommandData(jTokenMapped);
        }
        ...
        private void UpdateMatch(JToken jToken)
        {
            ...
            //if (jToken["streamingsrc"] != null) match.streamingsrc = jToken["streamingsrc"].ToString();
            //if (jToken["streamingid"] != null) match.streamingid = jToken["streamingid"].ToString();
            //if (jToken["streamingfixing"] != null) match.streamingfixing = jToken["streamingfixing"].ToString();
            //if (jToken["streamingofferto"] != null) match.streamingofferto = jToken["streamingofferto"].ToString();
            if (jToken["_f"] != null && jToken["_f"].ToString() == "2") match.HasStreaming = true;
            ...
            if (match.League.ToLower().Contains("saba soccer pingoal"))
            {
                return;
            }
        ...
        private void UpdateProduct(JToken jToken)
        {
            ...
            if (product.Bettype == Enums.BetType.FullTimeOverUnder)
            {
                Common.Functions.UpdateOverUnderScoreTimesFreq(product.MatchId, product);
                Common.Functions.UpdateOverUnderScoreTimesFreqBeforeLive(product.MatchId, product);
                Common.Functions.UpdateOverUnderScoreTimesFreqHalfTime(product.MatchId, product);
                
            }
            else if (product.Bettype == Enums.BetType.FirstHalfOverUnder)
            {
                Common.Functions.UpdateOverUnderScoreTimesFreqFh(product.MatchId, product);
                Common.Functions.UpdateOverUnderScoreTimesFreqFhBeforeLive(product.MatchId, product);
            }
            else if (product.Bettype == Enums.BetType.FullTimeHandicap)
            {
                Common.Functions.HandicapFreqBeforeLive(product.MatchId, product);
                Common.Functions.HandicapFreqHalfTime(product.MatchId, product);
            }
            else if (product.Bettype == Enums.BetType.FirstHalfHandicap)
            {
                Common.Functions.HandicapFreqFhBeforeLive(product.MatchId, product);
            }
