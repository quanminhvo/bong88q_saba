...
namespace LiveBetApp.Services.AutoBet
{
    ...
    public class Strategy_001_b
    {
        ...
        private Dictionary<long, bool> _matchBeginWithHdpsStepOneBetted_second; // <matchId, isBetted (step 1)>
        ...
        private void CleanUp()
        {
            ...
            matchIds = _matchBeginWithHdpsStepOneBetted_second.Keys.ToList();
            for (int i = 0; i < matchIds.Count; i++)
            {
                if (DataStore.Matchs[matchIds[i]].LivePeriod == -1)
                {
                    _matchBeginWithHdpsStepOneBetted_second.Remove(matchIds[i]);
                }
            }
        ...
        private void UpdateMatchsStartWith_Hdp()
        {
            ...
            for (int i = 0; i < matchs.Count; i++ )
            {
                    ...
                    && !_matchBeginWithHdpsStepOneBetted.ContainsKey(matchId))
                {
                    ...
                    _matchBeginWithHdpsStepOneBetted_second.Add(matchId, false);
        ...
        private void CheckAndPlaceBetStepOne(long matchId)
        {
            ...
            if (_matchBeginWithHdpsStepOneBetted_second.ContainsKey(matchId)
                && !_matchBeginWithHdpsStepOneBetted_second[matchId]
                && ((priceOfbeginHdp * priceOfTargetHdp > 0 && priceOfTargetHdp >= priceOfbeginHdp) || (priceOfbeginHdp > 0 && priceOfTargetHdp < 0)))
            {
                _matchBeginWithHdpsStepOneBetted_second[matchId] = true;
            ...
            {
                if (int.Parse(pricesStr[i]) >= boundary)
                {
                    _matchBeginWithHdpsStepOneBetted[matchId] = true;
                    if (! _matchBeginWithHdpsStepTwoBetted.ContainsKey(matchId)) _matchBeginWithHdpsStepTwoBetted.Add(matchId, false);
                    ...
                    DataStore.BetByHdpPrice.Add(new BetByHdpPrice() {
                        ...
                        CreateDateTime = DateTime.Now,
                        MoneyLine = DataStore.Matchs[matchId].OverUnderMoneyLine,
                        AutoBetMessage = "Strategy_001_b: kèo đầu tiên: " + beginHdp
            ...
            if (pricesStr.Count >= minuteCheck)
            {
                _matchBeginWithHdpsStepOneBetted[matchId] = true;
                if (!_matchBeginWithHdpsStepTwoBetted.ContainsKey(matchId)) _matchBeginWithHdpsStepTwoBetted.Add(matchId, false);
